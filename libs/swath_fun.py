...
def readKMALLswath(self, filename, print_updates=False, include_skm=False, parse_params_only=False, read_mode='full'):
	...
	data = {'fname': filename.rsplit('/')[-1],
			'HDR': km.mrz['header'],
			'RTP': km.mrz['pingInfo'],
			'cmnPart': km.mrz['cmnPart'],  # Add cmnPart to access swathsPerPing
			'IOP': km.iop,
			'IP': km.iip,
...
def adjust_depth_ref(det, depth_ref='raw data'):
	...
	# Check if detection dictionary is empty or has no soundings
	if not det or 'fname' not in det or len(det['fname']) == 0:
		print('WARNING: Empty detection dictionary in adjust_depth_ref, returning empty lists')
		return [], [], []
...
class kmall_data(kmall):
	...
	def extract_dg_optimized(self, dg_name, mode='full'):  # extract dicts of datagram types with mode selection
		"""
		Enhanced extract_dg with mode selection:
		- 'full': Complete datagram reading (current behavior)
		- 'plot': Plot-optimized reading (minimal data)
		- 'param': Parameter scanning only
		"""
		print('\n\nin extract_dg_optimized with dg_name = ', dg_name, ' and mode = ', mode)
		...
		if dg_name == 'MRZ' and mode == 'plot':
			return self.extract_dg_plot_mode()
		elif dg_name == 'MRZ' and mode == 'param':
			return self.extract_dg_param_mode()
		else:
			# Use existing extract_dg for other cases
			return self.extract_dg(dg_name)

	def extract_dg_plot_mode(self):
		"""
		Plot-optimized MRZ extraction - reads only essential data for plotting
		"""
	...
	def extract_dg_param_mode(self):
		"""
		Parameter scanning mode - extracts only parameter-related data
		"""
